/*******************************************************************************
  ...
  Objetivo     : Roteiro prático cobrindo criação e execução de User Defined Functions (UDFs) em PL/SQL, chamadas via DUAL e instruções DML, regras de escopo e uso de Global Temporary Tables (GTT) no Oracle Database 19c Multitenant (CDB/PDB).
...
-- Conceder privilégio de criação de Functions para o Schema USUARIO_TESTE
...
CREATE OR REPLACE FUNCTION usuario_teste.fn_ola (
    p_nome IN VARCHAR2
) RETURN VARCHAR2 IS
...
SELECT usuario_teste.fn_ola('World') AS mensagem FROM DUAL;
SELECT usuario_teste.fn_ola('Arley Ribeiro') AS mensagem_personalizada FROM DUAL;


-- Function com lógica condicional (IF...ELSE) para cálculo numérico
CREATE OR REPLACE FUNCTION usuario_teste.fn_obter_maximo (
    p_x IN NUMBER,
    p_y IN NUMBER
) RETURN NUMBER IS
    v_maximo NUMBER;
...
DECLARE
    ...
    v_max    NUMBER;
BEGIN
    v_max := usuario_teste.fn_obter_maximo(v_num1, v_num2);
...
CREATE OR REPLACE FUNCTION usuario_teste.fn_calcula_desconto (
    p_qtde             IN NUMBER,
...
SELECT usuario_teste.fn_calcula_desconto(1, 100, 0.1) AS valor_venda_final FROM DUAL;


-- Function com condicional ELSIF para categorização de clientes
CREATE OR REPLACE FUNCTION usuario_teste.fn_nivel_cliente (
    p_credito IN NUMBER
) RETURN VARCHAR2 IS
    v_nivel VARCHAR2(20);
BEGIN
...
BEGIN
    DBMS_OUTPUT.PUT_LINE('Credito 100:   ' || usuario_teste.fn_nivel_cliente(100));
    DBMS_OUTPUT.PUT_LINE('Credito 4999:  ' || usuario_teste.fn_nivel_cliente(4999));
    DBMS_OUTPUT.PUT_LINE('Credito 5000:  ' || usuario_teste.fn_nivel_cliente(5000));
    DBMS_OUTPUT.PUT_LINE('Credito 10000: ' || usuario_teste.fn_nivel_cliente(10000));
    DBMS_OUTPUT.PUT_LINE('Credito 10001: ' || usuario_teste.fn_nivel_cliente(10001));
END;
/


-- Refatoração utilizando a instrução CASE (Simplificação da lógica)
CREATE OR REPLACE FUNCTION usuario_teste.fn_nivel_cliente_case (
    p_credito IN NUMBER
) RETURN VARCHAR2 AS
BEGIN
    RETURN CASE
...
SELECT usuario_teste.fn_nivel_cliente_case(100)  AS resultado_100 FROM DUAL;
SELECT usuario_teste.fn_nivel_cliente_case(4999) AS resultado_4999 FROM DUAL;
...
SELECT 
    ...
    quantity, 
    unitprice,
    usuario_teste.fn_calcula_desconto(quantity, unitprice, 0.10) AS valor_com_desconto
FROM usuario_teste.orderitem;

-- Agregando o retorno de uma Function em tempo de execução (SUM + UDF)
SELECT 
    ROUND(SUM(usuario_teste.fn_calcula_desconto(quantity, unitprice, 0.10)), 2) AS total_vendas_com_desconto
FROM usuario_teste.orderitem;
...
CREATE GLOBAL TEMPORARY TABLE usuario_teste.gt_customer_berlin (
    id        NUMBER NOT NULL,
...
SELECT * FROM usuario_teste.gt_customer_berlin;

-- Limpeza total de dados e remoção da estrutura da tabela temporária
TRUNCATE TABLE usuario_teste.gt_customer_berlin;
DROP TABLE usuario_teste.gt_customer_berlin PURGE;


/* PARTE 6 - LIMPEZA DOS OBJETOS DO LABORATÓRIO (CLEANUP) */

DROP FUNCTION usuario_teste.fn_ola;
DROP FUNCTION usuario_teste.fn_obter_maximo;
DROP FUNCTION usuario_teste.fn_calcula_desconto;
DROP FUNCTION usuario_teste.fn_nivel_cliente;
DROP FUNCTION usuario_teste.fn_nivel_cliente_case;
