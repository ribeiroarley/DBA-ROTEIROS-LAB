/*******************************************************************************
  ...
  Objetivo     : Laboratório prático sobre Arquitetura Multitenant (CDB/PDB), gerenciamento de PDBs, Usuários Comuns/Locais e Privilégios no Oracle Database 19c.
...
CREATE USER c##usuario_teste IDENTIFIED BY "commonuser123" CONTAINER=ALL;

-- Conceder permissão de conexão global
GRANT CONNECT TO c##usuario_teste CONTAINER=ALL;

-- Conceder privilégio de criar tabelas apenas no container atual (CDB$ROOT)
GRANT CREATE TABLE TO c##usuario_teste CONTAINER=CURRENT;

-- Teste de conexão com o usuário comum
CONNECT c##usuario_teste/commonuser123;
SHOW con_name;
SHOW user;
...
CONNECT / AS SYSDBA;
REVOKE CREATE TABLE FROM c##usuario_teste CONTAINER=CURRENT;
...
CREATE USER usuario_teste2 IDENTIFIED BY "teste123" CONTAINER=CURRENT;
GRANT CONNECT TO usuario_teste2 CONTAINER=CURRENT;
...
CREATE PLUGGABLE DATABASE pdb1 
  ADMIN USER teste IDENTIFIED BY "benvindo123" 
  STORAGE (MAXSIZE 2G);
...
-- Criar Schema/Usuário 'TESTE' que atuará como dono dos objetos
...
-- Criar Usuário 'USUARIO_TESTE' que consumirá os objetos do Schema TESTE
CREATE USER usuario_teste IDENTIFIED BY "teste123" CONTAINER=CURRENT;
GRANT CONNECT TO usuario_teste CONTAINER=CURRENT;

-- Conectar como TESTE e criar a estrutura da tabela
...
-- Conceder privilégios granulares na tabela CLIENTE para o usuário USUARIO_TESTE
GRANT SELECT, INSERT, UPDATE, DELETE ON teste.cliente TO usuario_teste;

-- Conectar como USUARIO_TESTE e manipular os dados do Schema TESTE
CONNECT usuario_teste/teste123@//localhost:1521/ORCLPDB;

-- Manipulação DML
SELECT * FROM teste.cliente;
UPDATE teste.cliente SET nome = 'Teste Alterado' WHERE matricula = 1;
DELETE FROM teste.cliente WHERE matricula = 1;
COMMIT;
...
CONNECT teste/teste123@//localhost:1521/ORCLPDB;
REVOKE ALL ON teste.cliente FROM usuario_teste;

-- Exemplo de Concessão de Privilégio Global de Leitura (Usar com cautela em produção)
CONNECT / AS SYSDBA;
ALTER SESSION SET CONTAINER = ORCLPDB;
GRANT SELECT ANY TABLE TO usuario_teste;
...
CONNECT / AS SYSDBA;
ALTER SESSION SET CONTAINER = ORCLPDB;
REVOKE SELECT ANY TABLE FROM usuario_teste;
...
DROP USER usuario_teste2 CASCADE;
DROP USER c##usuario_teste CASCADE;
